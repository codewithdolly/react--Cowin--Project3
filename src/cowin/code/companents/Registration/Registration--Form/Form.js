...
import { color, display, fontWeight, width } from '@mui/system';
...
export default function BasicCard() {
  ...
  const handleClick=()=>{
    ...
    console.log(state)
    axios.post('http://localhost:8080/cowin-trial-otp',mobile )
        .then(response => console.log(response));
  }

  const displayNum=()=>{
    axios.get('http://localhost:8080/cowin-trial-otp')
    .then((res)=>{
  ...
  return (
    ...
    <Card sx={{ minWidth: 275,marginTop:10, marginLeft:50 , marginRight:50 }} className='main--card'>
      ...
      <CardContent >
        <div className='otp-logo'style={{width:320, backgroundColor:'#fff',borderRadius:80}}>
      <Avatar  sx={{width:80, height:70, ml:20}} alt="Travis Howard"  src={rlogo} alt='30'></Avatar>
        ...
        <TextField value={state}  onChange={handleChange} sx={{ml:12}}id="standard-basic" label="Enter Your Mobile Number" variant="standard" />
      </CardContent>
      <CardActions>
        <div style={{flexDirection:'Column'}}>
      <Button  onClick={handleClick} sx={{width:350,ml:4}} variant="contained">Get Otp</Button>
    ...
    <div style={{width:200,m:5}}>

        <Button onClick={displayNum} sx={{width:100,mt:10}} >getNumber</Button>
